...
opus_to_dataset <- function(SPECPATH ="/Data_Raw/SPECTRA", NWAVE=3017, SAVENAME="none"){
  ...
  #spc.df$sample_id <- str_sub(spc.df$sample_id,1,str_length(spc.df$sample_id)-10) -- truncates sample id, I don't need that
  #spc.avg.df <- aggregate(.~sample_id, data = spc.df, FUN=mean, na.rm=TRUE) -- Zack does not know why this was commented out
...
subset_spectral_range <- function(SPECTRA){
  ...
  col.names <- as.numeric(substring(col.names,2)) #column names all start with X, this trims that to just spectra numbers
  
  cutoff <- which(col.names <= 628)[1]
  
  if(!is.na(cutoff)){
    SPECTRA <- SPECTRA[,-c(cutoff:length(col.names))] #truncate at >= 628 Zack added the if statement because our data doesn't have spectra below 628
  }
...
refineSpecLib <- function(SPECLIB, PROP=NA, OUTLIER=c("stdev","fratio"), LARGE=FALSE, CALVAL=FALSE, SAVENAME="none"){
  ...
  if(("fratio" %in% OUTLIER)){
    SPECLIB  <-  fratio_outliers(SPECLIB)# Identified with fratio-  original was SPECLIB[-fratio_outliers(SPECLIB),] but when there were no outliers it cut our data
  } 
