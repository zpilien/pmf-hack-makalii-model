...
stdev_outliers <- function(SPECLIB, PROP, SHOW=TRUE, PLOT=TRUE){
  ...
  pls.fit <- plsr(sqrt(get(PROP))~spc, ncomp= 20, data = SPECLIB, valid="CV", segments = 50) #y, x, number components, data, cross validation, -- ncomp used to be 20 but I got error
...
outlier_indices <- function(x,y,sd,.....)
{
  ...
  #newd <- datum[datum$sqrt.oc.10.comps<upr & datum$sqrt.oc.10.comps>lwr,] - Zack did not do this I dunno why it's commented out
  return(outlier.rows)
}
...
fratio_outliers <- function(SPECLIB, P=0.99, SHOW=TRUE, PLOT=TRUE){
  ...
  if(length(outliers)>0){
    ...
    return(SPECLIB[-fratio_outliers(SPECLIB),])
    #return(outliers)
  }else{
    print("No Spectral Outliers")
    return(SPECLIB) #original script did not have a return statement here causing an error
  }
