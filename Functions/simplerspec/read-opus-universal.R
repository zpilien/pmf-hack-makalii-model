...
read_opus_bin_univ <- function(file_path, extract = c("spc"),
  ...
  if(!file.exists(file_path)) {
    stop(paste0("File does not exist"))  #read_opus_univ
  }
  ...
  try({
	...
	#Fix Bug 1: Fix bug with positions related to end strings
	#if (length(end) > 25) end <- end[-14]

    # Get all positions of "NPT" (number of points) string
    npt_all <- grepRaw("NPT", pr, all = TRUE) + 3

	#bad way to fix the bug for reading some opus file -- somehow the length of npt_all opus file has varying length which is fixed here -- added by Shree Dangal on 5/31/2018
	if(length(npt_all) > 7) {
	npt_all <- npt_all[2:length(npt_all)]
	}
	...
	#cat(end_spc,"\n")
	#cat(NPT_spc, "\n")
	if(length(end_spc) > length(NPT_spc)) end_spc <- end_spc[1:length(NPT_spc)]
    ...
    # Assign single channel spectra if present in file -------------------------  cutoff
